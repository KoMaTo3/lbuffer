...
public:
  ...
  template < class T >
  static bool         Between( T l, T r, T x, float epsilon );
  template < class T >
  static bool Intersect1D( T a, T b, T c, T d, float epsilon = 0.01f );
...
KM_INLINE static bool Math::Between( T _l, T _r, T _x, float epsilon ) {
  //printf( "BW: %3.10f : %3.10f : %3.10f : result[%d]\n", _l, _r, _x, ( min( _l, _r ) <= _x + epsilon && _x <= max( _l, _r ) + epsilon ) );
...
KM_INLINE static bool Math::Intersect1D( T a, T b, T c, T d, float epsilon ) {
  ...
  //printf( "Intersect1D[%3.5f; %3.5f]-[%3.5f; %3.5f]\n", a, b, c, d );
